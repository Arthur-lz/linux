...
> 3.要考虑负荷权重对CPU负载的不同贡献，原始CPU负载根据负荷权重进行比例绽放后得到最终的CPU负载
...
> 旧版本内核之所以使用cpu_load[]数组，是因为它考虑了历史贡献和衰减效应，变化相对平稳。从PELT机制引入以后，CPU负载因子中记录的平均CPU负载水平本来就是考虑了历史贡献和衰减效应的，在cpu_load[]里再处理一遍只会带来额外的复杂度而并没有太大意义。因此，linux 5.4.x版本内存在cpu之间进程负载均衡操作的时候，直接考虑cpu_runnable_load()就可以，该函数返回的是CFS运行队列中可运行进程贡献的平均cpu负载，相当于旧内核的cpu_load[0]
...
* 进程迁移
> 内存迁移（让内存跟着进程跑）相对来说代价较大，只有在非常必要的时候才会使用。相比之下进程迁移代价较小，在此调度器考察进程的内存分布以后，如果发现进程与其使用的大部分内存不在同一NUMA节点时，就会迁移进程（让进程跟着内存跑）
