...
void init_wakeup(void)
{
	...
	prepare_to_wait_exclusive(&head, &data3, 1);//将新进程加入等待队列，并将当前进程状态改为1, 这里只是单纯的设置一下当前进程的状态，这种操作并不会改变当前进程的执行状态
	...
	printk(">>>current id:%d, state:%d, DEL>>id:%d, state:%d\n", current->pid, current->state,
	...
	finish_wait(&head, &data1); // 设置当前进程状态为0，并将进程data1从head中删除
	printk("after>>>current id:%d, state:%d, DEL>>id:%d, state:%d\n", current->pid, current->state,
			((struct task_struct*)(data1.private))->pid,
			((struct task_struct*)(data1.private))->state
			);
